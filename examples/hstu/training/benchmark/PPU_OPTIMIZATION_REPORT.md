...
> 对照: H20 (SM 9.0, 132 SMs, 989 BF16 TFLOPS) × 16 卡
...
| 版本 | TFLOPS/GPU | MFU | vs H20 | vs 原始 |
|------|---:|---:|---:|---:|
| 原始框架 (无优化) | 229.64 | 14.60% | 75.9% | — |
| + cuBLAS GEMM | 232.08 | 14.76% | 76.7% | +1.06% |
| + CUTLASS Tile 调优 | 232.44 | 14.78% | 76.8% | +1.22% |
| H20 (参考) | 302.6 | 30.59% | 100% | — |

**硬件理论上限**: PPU 峰值 / H20 峰值 = 787 / 989 = **79.6%**

**PPU 相对效率**: 76.8% / 79.6% = **96.5%**（已达到硬件理论上限的 96.5%）
...
2. **PPU 已达硬件天花板的 96.5%**: 实际 76.8% vs 理论 79.6%（相对 H20）
3. **框架层面几乎无进一步优化空间**: GEMM、attention kernel、autotune 均已优化到位
4. **剩余差距来自硬件**: SM 数量（64 vs 132）、显存带宽、缺失 TMA/wgmma 等 Hopper 特性
